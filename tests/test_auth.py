...
async def test_register_default_role_student(client):
    resp = await helpers.register_user(client, "alice", "pass123")
...
async def test_register_with_hr_role(client):
    resp = await helpers.register_user(client, "boss", "pass123", role="hr")
    assert resp.status_code == 201
    assert resp.json()["role"] == "hr"


async def test_register_invalid_role(client):
    resp = await helpers.register_user(client, "bad", "pass123", role="admin")
    assert resp.status_code == 422


async def test_register_duplicate_username(client):
    await helpers.register_user(client, "alice", "pass123")
    resp = await helpers.register_user(client, "alice", "pass123")
...
async def test_login_success(client):
    await helpers.register_user(client, "alice", "pass123")
    resp = await helpers.login_user(client, "alice", "pass123")
...
async def test_login_nonexistent_user(client):
    resp = await helpers.login_user(client, "ghost", "pass123")
    assert resp.status_code == 401
